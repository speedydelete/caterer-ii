...
async function runSim(argv: string[], rle: string): Promise<[number, string | undefined]> {
    ...
    for (let {p, time} of frames) {
        ...
        let pData = p.getData();
        let index = 0;
        gifData.push(new Uint8Array([0x21, 0xf9, 0x04, 0x09, time & 255, (time >> 8) & 255, 0xff, 0x00, 0x2c, 0x00, 0x00, 0x00, 0x00, width & 255, (width >> 8) & 255, height & 255, (height >> 8) & 255, 0x00, bitWidth]));
        let data: number[] = [];
